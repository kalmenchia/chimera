...
type
  ...
  TIdOnChunkReceived = procedure(Sender : TObject; Chunk : TStream) of object;
...
var
  ...
  var
    LSize: Integer;
    LTrailHeader: String;
    LChunkStream : TMemoryStream;
  begin
    if Assigned(FOnChunkReceived) then
      LChunkStream := TMemoryStream.Create
    else
      LChunkStream := nil;
    try
      ...
      try
        ...
        while LSize <> 0 do begin
          if Assigned(LS) then begin
            if Assigned(LChunkStream) then begin
              IOHandler.ReadStream(LChunkStream, LSize);
              LChunkStream.Position := 0;
              FOnChunkReceived(Self, LChunkStream);
              LChunkStream.Position := 0;
              LS.Write(LChunkStream.Memory^, LSize);
              LChunkStream.Size := 0;
            end else
